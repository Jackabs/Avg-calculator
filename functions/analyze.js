...
exports.handler = async (event) => {
  try {
    const { oldAvg, oldQty, newPrice, mode, targetAvg, newQty, asset } = JSON.parse(event.body);
    ...
    const spotPrice = parseFloat(alphaData['Realtime Commodity Exchange Rate']['5. Exchange Rate']);

    // 3) Build AI prompt
const prompt = `
You are a financial analyst. Current ${asset.toUpperCase()} spot price in INR is ₹${spotPrice}.
The user wants to ${mode === 'target'
...
// 4) Call OpenAI
  ...
  // 5) Return both our calculator suggestion and the AI’s rationale
  return {
    statusCode: 200,
    body: JSON.stringify({
